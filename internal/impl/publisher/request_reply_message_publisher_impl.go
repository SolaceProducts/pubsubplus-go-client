...
func (publisher *requestReplyMessagePublisherImpl) Terminate(gracePeriod time.Duration) (err error) {
    ...
    if publisher.backpressureConfiguration != backpressureConfigurationDirect {
        ...
        undeliveredCount := publisher.drainQueue(time.Now(), nil)
...
func (publisher *requestReplyMessagePublisherImpl) drainQueue(shutdownTime time.Time, err error) uint64 {
    ...
    for range publisher.buffer {
        ...
        */
        // handle each message by correlation id remove remove each entry as a pending request then submit replyHandler with err
    }
...
func (publisher *requestReplyMessagePublisherImpl) PublishBytes(bytes []byte, replyMessageHandler solace.ReplyMessageHandler, dest *resource.Topic, replyTimeout time.Duration, userContext interface{}) error {
    ...
    return publisher.publish(msg.(*message.OutboundMessageImpl), replyMessageHandler, dest, replyTimeout, userContext)
}
...
func (publisher *requestReplyMessagePublisherImpl) Publish(msg apimessage.OutboundMessage, replyMessageHandler solace.ReplyMessageHandler, dest *resource.Topic, replyTimeout time.Duration, properties config.MessagePropertiesConfigurationProvider, userContext interface{}) error {
    ...
    return publisher.publish(msgDup, replyMessageHandler, dest, replyTimeout, userContext)
}
...
func (publisher *requestReplyMessagePublisherImpl) publish(msg *message.OutboundMessageImpl, replyMessageHandler solace.ReplyMessageHandler, dest *resource.Topic, replyTimeout time.Duration, userContext interface{}) (ret error) {
    ...
    // TODO set the message replyTo destination
    
    // TODO set the message correlation id
...
func (publisher *requestReplyMessagePublisherImpl) sendTask(msg *message.OutboundMessageImpl, dest resource.Destination) buffer.PublisherTask {
            ...
            if ok {
                ...
                if errorInfo != nil /* && requestHandler != nil */ {
                    ...
                    if !publisher.eventExecutor.Submit(func() { /* call request specific reply handler */  }) &&
                        ...
                        publisher.logger.Info(fmt.Sprintf("Failed to submit publish reply handler callback for correlation %v. Is the publisher terminated?", 0 /* correlation id */))
                    }
                } else {
